...
export const Mutation = {
  login: (_parent: any, loginData: LoginUser, context: Context) =>
    authentication.loginUser(context, loginData),
  loginWithMail: (_parent: any, emailLoginId: string, context: Context) =>
    inviteTransactions.activateAccount(context, emailLoginId),
  sendLoginLink: (_parent: any, identifier: string, context: Context) =>
    authentication.sendLoginLink(context, identifier),
  voteCity: (_parent: any, cityVote: VoteCity, context: Context) =>
    cityTransactions.voteCity(context, cityVote),
  invite: async (_parent: any, invite: any, context: Context) =>
    inviteTransactions.createInvitation(context, invite),
