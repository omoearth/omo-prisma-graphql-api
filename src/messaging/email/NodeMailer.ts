...
import { prisma, Invitation, User, City, EmailLogin } from '../../generated/prisma-client';
...
export class NodeMailer {
	...
	private async sendMailTemplate(templateName: string, to: string, replacements: Map<string, string>) {
	...
	async sendLoginLink(emailLogin: EmailLogin, user: User) {
		let replacements = new Map<string, string>();
		replacements.set('$-{user}', user.name || '');
		replacements.set('$-{link}', `${process.env.EARTH_HTTP_ENDPOINT}/login/${emailLogin.id}`);
		return this.sendMailTemplate('LOGINWITHMAIL', user.identifier || '', replacements);
	}
}
